# Repository of the Api(v 0.5)

## What Api do now
- Returns FirsName, LastName, login and password of specific user by his id
- Can Create a new user and save him in database
- Can generate token save it check if it is actual and update if not
- Can login by token and login and password
- Search game by filters and sort it
- Getting all cities in database
- Create new game
- Getting all tags in database
 
## List of Tasks
- [x] Integrate with Render
- [x] Sign up of user
- [x] Log in of user
- [x] Tokens generating, sending, saving and checking / Sessions
- [x] Searching of games by many filter and by sorting
- [x] Geting all cities
- [x] Creating new games
- [x] Geting all tags

## Api directory structure
- In root directory we have requirements.txt, which contains necessary libraries for corect work of api.
- StartApp.bat it's a file in the windows system language, which creates python virtual eviroment and starts api on local machine. It can also work with aiven database but u have to change database login data in api.
- Next is database directory, which contains config file. This file includes configuration and connection of sqlAlchemy with local or cloud database. To change with which database we want to connect just set isLocal variable to true or false.
- Next in root directory we have api directory, which contains the most important files in all root directory, but first we will start with models directory content. It has the model file which contains, as name say, models of tabels in database. It improves work with database and it is necessary for sqlAlchemy to working properly. It isn't hand written, it was generated by sqlacodegen library. This lib is escpesialy created generating tool for sqlAlchemy, so i prefer to start api by full locally to generate new models if database was change.
- Now we have to go back to api directory, which has the main file. This file contains full working api code. There are created functions for every specific url in api, so read it cerfully to avoid mistakes and misscalculation in your work of api.    
 
> [!IMPORTANT]
> **How to start up Api on your system**
> 1. First of all you need to import database to your phpmyadmin, name it erasmus. Database is in db directory
> 2. You have to have user name root without password and your server must running on localhost
> 3. If u are doing it on local machine u have to go to ./api/database/config.py and change variable isLocal to true
> 4. In this step all you need to do is double click on file named StartApp.bat it will lunch Api server with all dependency
> 5. Api will start on 127.0.0.1:8000/docs
> 
> **How to start up Api in network if not working**
> 1. You have to go on this website "https://aiven.io/" and log in by **mail: elektrykerasmus@gmail.com password: jyhCo6-qesqor-syhhor**
> 2. Next you have to click on three dots and  click power on service
> 3. Next go to website "https://render.com/" and login with the same data, that were used in aiven
> 4. Next click service named ErasmusApi, click Manual Deploy and click deploy latest commit
> 5. Last just copy and paste the url that is showed on the same page under name of service 

> [!NOTE]
> **If u have trouble with api, just write it on discord server of erasmus in tab Api chat reports**

> [!TIP]
> **Remeber that the Swager window of the api is on  api_url/docs**
